...
python do_image_size_check() {
    imgfile = d.getVar("RESIN_DOCKER_IMG")
    ext4file = d.getVar("RESIN_ROOTB_FS")
    ...
    available = int(disk_aligned(d, available_space(ext4file))) - int(rfs_alignment)
    ...
    bb.debug('requested %d, available %d' % (image_size_aligned, available) )
}
